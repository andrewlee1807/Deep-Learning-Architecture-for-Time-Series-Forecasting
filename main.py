...
from utils import AreaEnergy
from utils import HouseholdDataLoader
from utils import TSF_Data

공대7호관_HV_02 = AreaEnergy(
    "공대7호관.HV_02",
    path_time=r"C:\Users\Andrew\Documents\Project\Time Series\dataset/Electricity data_CNU/3.unit of time(일보)/",
)
...
def model_builder(hp):
    ...
    model = StrideDilatedNet(
        list_stride=(layer_stride1, layer_stride2),
    ...
    model.compile(
        loss=tf.keras.losses.Huber(), optimizer="adam", metrics=["mse", "mae"]
    )
...
for output_width in range(1, 25):
    ...
    pd.DataFrame.from_dict(history.history).to_csv(
        result_path + "/history" + str(output_width) + ".csv", index=False
    )
