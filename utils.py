...
class TSF:
    ...
    def export_label_array(self, data_pack):
        y_label = (
            ...
            else np.empty((0, self.label_width))
        )
...
class AreaEnergy:
    ...
    def __init__(self, name_area, path_time=None):
        ...
        index_datatime = np.array(
            list(
                map(
                    lambda x: [
                        datetime.datetime(int(x[:4]), int(x[4:6]), int(x[6:8]), i)
                        for i in range(24)
                    ],
                    time_index,
                )
    ...
    def extract_observation(self, df):
        ...
        # print(columns_name)
        df_sub = df.loc[
            :, ~df.columns.isin(["Unnamed: 0", "Unnamed: 2", "Unnamed: 27"])
        ]
        df_sub.columns = columns_name
        df_sub = df_sub.reset_index().drop(columns=["index"])
        record = df_sub[df_sub["차단기 명"] == self.name_area]
        self.arr_seq_dataset = np.append(
            self.arr_seq_dataset, np.squeeze(record.to_numpy())[1:]
        )
    ...
    def plot_sequence(self, df_seq=None):
        ...
        if df_seq is None:
            df_seq = pd.DataFrame(
                self.arr_seq_dataset, columns=["전력사용량"], index=self.index_all_time
            )
...
class SpainDataLoader:
    def __init__(self, data_path=None):
        ...
        self.files = [
            self.data_path + "/" + "20201015_" + name + ".xlsx"
    ...
    def scale_data(self, data):
        ...
        x = x.append(
            pd.DataFrame(x.tail(1), index=x.tail(1).index + pd.Timedelta(days=1))
        )
    ...
    def load_data(self):
        consumptions = pd.read_excel(
            self.files[self.categories.index("consumption")],
            parse_dates=[0],
            index_col=0,
        )
        consumptions.columns = pd.DataFrame(
            consumptions.columns, columns=["customer"]
        ).index
        ...
        weather = pd.read_excel(
            self.files[self.categories.index("weather")],
            parse_dates=[0],
            index_col=0,
        )
    ...
    def prepare_data(self, consumptions, weather, holidays):
        ...
        days = pd.DataFrame(
            pd.to_datetime(consumptions.index.date),
        ...
        days["day_category"] = days["day_of_week"].replace(
            {0: 0, 1: 1, 2: 1, 3: 1, 4: 2, 5: 3, 6: 4}
        )
        days.loc[days["date"].apply(lambda d: d in holidays), "day_category"] = 4
        days = days.groupby(["date"]).first()
        consumptions_daily_mean = pd.DataFrame(
            consumptions.groupby(consumptions.index.date).mean(), index=days.index
        )
        weather_daily_mean = pd.DataFrame(
            weather.groupby(weather.index.date).mean(), index=days.index
        )
        return consumptions_daily_mean, weather_daily_mean, days

    def plot_sequence(self, customer=20):
        fig, axs = plt.subplots(figsize=(25, 8))
        _ = self.consumptions.loc[:, customer].plot(
            color="purple",
            alpha=0.7,
